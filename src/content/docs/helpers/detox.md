...
> Note: When combining these flows with web UI steps, ARIA locators like `{ aria: 'Sign in' }` are supported by web helpers (Playwright, WebDriver, Puppeteer).
...
This is a wrapper on top of [Detox][1] library, aimed to unify testing experience for the CodeceptJS framework.
Detox provides grey box testing for mobile applications and works especially well for React Native apps.
...
This is why **Detox is a grey box testing** solution: you need access to application source code and a way to build and execute it on an emulator.

Compared to Appium, Detox runs faster and is more stable but requires additional setup for builds.
...
If you completed step 1 and step 2, you should have a configuration similar to this:
