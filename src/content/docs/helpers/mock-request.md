...
*   passthrough (default) - mock predefined HTTP requests
...
To intercept API requests and mock them, use the following API:
...
Record & Replay mode allows you to record all XHR and fetch requests and save them to a file.
...
Set mode via environment variable, `replay` mode by default:
...
I.startMocking();
I.mockServer((server) => {
  // mock request only from api1.com and api2.com and
...
To update [PollyJS configuration][14] use the second argument:
...
Must be called to save recorded requests into file.
